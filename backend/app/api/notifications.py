...
from app.models.models import NotificationChannelModel, NotificationLog
...
async def list_channels(project_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def create_channel(project_id: UUID, data: NotificationChannelCreate, db: AsyncSession = Depends(get_db)):
...
async def update_channel(channel_id: UUID, data: NotificationChannelUpdate, db: AsyncSession = Depends(get_db)):
...
async def test_notification(channel_id: UUID, db: AsyncSession = Depends(get_db)):
